...
if (isset($_POST['id_projeto']) && $_POST['id_projeto'] != 0) {
    ...
    $statement = $conn_meta->prepare("INSERT INTO servico(id_cliente, recebido_por, pedido_por, data_hora, observacoes, prioridade, tipo_servico, id_proj_cliente, estado, is_active,local_partida, local_chegada, distancia, duracao, distanciaAestrada, duracaoAestrada)"
            . " VALUES ($id_cliente, '" . $_SESSION['nome'] . "','$pedido_por', '$timestamp', '$observacoes', $prioridade, 2, $id_proj_cli, 0, 1, 'Metaveiro', '$local_chegada', '$distancia_semauto', '$duracao_semauto', '$distancia', '$duracao')");


    $statement->execute();

    $id_lastAssistencia = $conn_meta->lastInsertId();
} else {
    ...
    //insert tab servico uma Assistencia
    $statement = $conn_meta->prepare("INSERT INTO servico(id_cliente, recebido_por, pedido_por, data_hora, observacoes, prioridade, tipo_servico, id_proj_cliente, estado, is_active, local_partida, local_chegada, distancia, duracao, distanciaAestrada, duracaoAestrada)"
            . " VALUES ($id_cliente, '" . $_SESSION['nome'] . "','$pedido_por', '$timestamp', '$observacoes', $prioridade, 2, NULL, 0, 1, 'Metaveiro', '$local_chegada', '$distancia_semauto', '$duracao_semauto', '$distancia', '$duracao')");
...
if (isset($_POST['selecionados'])) {
    ...
    foreach ($areas_filtradas as $areas) {
        if ($i == $len - 1) {
            $insertAreas .= "($areas,$id_lastAssistencia, 1)";
        } else {
            $insertAreas .= "($areas,$id_lastAssistencia, 1),";
        }
        $i++;
...
$json = json_encode(array('id_assistencia' => $id_lastAssistencia));
echo $json;
?>