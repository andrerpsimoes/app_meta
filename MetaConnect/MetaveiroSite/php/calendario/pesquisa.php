...
if( $conn ) {
    ...
    $sql = "select intNumero, strCVDNome, CA_Concluido, dtmData from Mov_Venda_Cab where strAbrevTpDoc= 'ASIST' and dtmData BETWEEN '".$final."' AND '".$dataatual."' order by dtmDataAlteracao desc ";
...
while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
  ...
  if($row['CA_Concluido']=="0"){
    ...
    /*else
  echo "<option  style=\"background: #5cb85c ; color: #fff;\" value=\"".$row['intNumero']."-".utf8_encode($row['strCVDNome'])."\">".$row['intNumero']." - ".utf8_encode($row['strCVDNome'])."</option>";    
*/
}
